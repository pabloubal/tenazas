...
- **Status State Machine**: `ValidateStatusTransition(from, to)` enforces a strict transition graph (e.g., `todo → in-progress | blocked | done`). Status changes via `work edit --status` are validated before persistence.
