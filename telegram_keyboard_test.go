...
func TestExtractShellCommand(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "Bash block",
			content:  "Here is the command:\x0a" + "```bash\x0als -la\x0a```" + "\x0aLet me know if it works.",
			expected: "ls -la",
		},
		{
			name:     "Sh block",
			content:  "Try this:\x0a" + "```sh\x0aecho 'hello world'\x0a```",
			expected: "echo 'hello world'",
		},
		{
			name:     "Shell block",
			content:  "```shell\x0amkdir -p test/dir\x0a```",
			expected: "mkdir -p test/dir",
		},
		{
			name:     "No language block",
			content:  "Run this:\x0a```\x0acat file.txt\x0a```",
			expected: "cat file.txt",
		},
		{
			name:     "Multiple blocks - picks first",
			content:  "First:\x0a" + "```bash\x0acd src\x0a```" + "\x0aSecond:\x0a" + "```bash\x0als\x0a```",
			expected: "cd src",
		},
		{
			name:     "No blocks",
...
func TestGetActionKeyboard(t *testing.T) {
	...
	t.Run("With command", func(t *testing.T) {
		content := "I recommend running:\x0a" + "```bash\x0agit status\x0a```"
		markupRaw := getActionKeyboard(sessionID, content)
	...
	t.Run("With long command - shortened", func(t *testing.T) {
		content := "```bash\x0adocker run -it --rm -v $(pwd):/app node:18 npm install && npm run build\x0a```"
		markupRaw := getActionKeyboard(sessionID, content)
