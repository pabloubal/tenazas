...
func (m *mockEngine) ExecutePrompt(sess *Session, prompt string) {
	m.executePromptCalled = true
	m.lastPrompt = prompt
}

func (m *mockEngine) ExecuteCommand(sess *Session, cmd string) {
	m.executeCommandCalled = true
	m.lastCommand = cmd
}

func (m *mockEngine) Run(skill *SkillGraph, sess *Session)  {}
...
func TestHandleActionCallback(t *testing.T) {
	...
	t.Run("Run Command", func(t *testing.T) {
		...
		sm.Log(sess, AuditLLMResponse, "Here is a command:\x0a```bash\x0als -l\x0a```")
