...
func TestSkillIsolation_Loading(t *testing.T) {
	...
	path := storage.ResolveSkillPath(skillName)
...
func TestResolveInstruction_PathSafety(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// ResolveInstruction needs to be implemented/updated in storage.go or session.go
			_, err := ResolveInstruction(tt.path, tt.skillBaseDir, tt.sessionCWD)
