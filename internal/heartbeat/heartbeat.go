...
// Notifier is the interface for sending notifications (implemented by Telegram).
type Notifier interface {
	SendNotification(chatID int64, text string)
...
func NewRunner(configDir string, sm *session.Manager, eng *engine.Engine, notifier Notifier) *Runner {
	return &Runner{
		configDir: configDir,
		sm:        sm,
		engine:    eng,
...
func (h *Runner) RunLoop(hb models.Heartbeat) {
...
func (h *Runner) blockTask(hbName string, t *task.Task) {
	...
	if h.notifier != nil {
		chatIDs := h.notifier.AllowedChatIDs()
		if len(chatIDs) > 0 {
			h.notifier.SendNotification(chatIDs[0], msg)
		}
	}
}
...
func (h *Runner) runSkillHeadless(hbName, skillName, cwd string, activeTask *task.Task) error {
	...
	if latest != nil && latest.Status != models.StatusCompleted && latest.Status != models.StatusFailed {
		sess = latest
	} else {
		sess = &models.Session{
			ID:        uuid.New().String(),
...
func (h *Runner) resolveTasksDir(hbPath string) string {
	hbTasksDir := filepath.Join(h.sm.Storage.BaseDir, "tasks", storage.Slugify(hbPath))

	if _, err := os.Stat(hbTasksDir); os.IsNotExist(err) {
		cwd, _ := os.Getwd()
		cwdTasksDir := filepath.Join(h.sm.Storage.BaseDir, "tasks", storage.Slugify(cwd))
