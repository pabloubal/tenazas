...
func TestNotifyTaskState(t *testing.T) {
	...
	t.Run("Update Notification (EditMessage)", func(t *testing.T) {
		...
		tg.NotifyTaskState(sess.ID, events.TaskStateBlocked, map[string]string{"reason": "Max retries reached"})
...
func TestHandleTaskCallbacks(t *testing.T) {
	...
	t.Run("Task Pause", func(t *testing.T) {
		...
		if updatedSess.Status != models.StatusIdle {
			t.Errorf("Expected status to be %s, got %s", models.StatusIdle, updatedSess.Status)
		}
	})
