...
func TestTelegramUXOverhaul(t *testing.T) {
	...
	t.Run("Context-Aware Keyboards - With Bash", func(t *testing.T) {
		...
		tg.broadcastAudit("sess-123", events.AuditEntry{
			Type:    events.AuditLLMResponse,
			Content: content,
	...
	t.Run("Context-Aware Keyboards - No Command", func(t *testing.T) {
		...
		tg.handleStreamingEnd(chatID, "sess-123", "Just text.", &formatter.HtmlFormatter{})
	...
	t.Run("Task Monitoring & Upsert", func(t *testing.T) {
		...
		tg.NotifyTaskState(sess.ID, events.TaskStateStarted, nil)
		...
		tg.NotifyTaskState(sess.ID, events.TaskStateBlocked, map[string]string{"reason": "Wait"})
