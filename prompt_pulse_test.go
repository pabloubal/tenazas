...
func TestThinkingStateTransition(t *testing.T) {
	...
	exec := NewExecutor("gemini", t.TempDir())
	...
	GlobalBus.Publish(Event{
		SessionID: sessionID,
		Type:      EventAudit,
		Payload:   AuditEntry{Type: AuditLLMPrompt},
	})
	...
	// but here we are in 'package main', so we can if we add them.
	...
	GlobalBus.Publish(Event{
		SessionID: sessionID,
		Type:      EventAudit,
		Payload:   AuditEntry{Type: AuditLLMChunk, Content: "hi"},
	})
